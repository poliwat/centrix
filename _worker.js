/**
 * Centrix Production Assessment Worker
...
export default {
  async fetch(request, env) {
    // Only handle POST requests to /api/assessment
    if (request.method === 'POST' && new URL(request.url).pathname === '/api/assessment') {
      return handleAssessment(request, env);
    }

    // Enable CORS for preflight requests
    if (request.method === 'OPTIONS' && new URL(request.url).pathname === '/api/assessment') {
    ...
    // Let other requests pass through to Pages
